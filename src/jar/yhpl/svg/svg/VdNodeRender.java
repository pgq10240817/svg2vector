...
/**
 * Given an array of VdPath.Node, generate a Path2D object. In another word,
 * this is the engine which converts the pathData into a Path2D object, which is
 * able to draw on Swing components. The logic and math here are the same as
 * PathParser.java in framework.
 */
class VdNodeRender {
	...
	private static void addCommand(Path2D path, float[] current, char cmd, char lastCmd, float[] val) {
		...
		for (int k = 0; k < val.length; k += incr) {
			...
			case 'V':
				...
				break;
			case 'c':
				path.curveTo(cx + val[k + 0], cy + val[k + 1], cx + val[k + 2], cy + val[k + 3], cx + val[k + 4], cy
						+ val[k + 5]);
				cpx = cx + val[k + 2];
				cpy = cy + val[k + 3];
				cx += val[k + 4];
			...
			case 's':
				...
				path.curveTo(reflectCtrl ? 2 * cx - cpx : cx, reflectCtrl ? 2 * cy - cpy : cy, cx + val[k + 0], cy
						+ val[k + 1], cx + val[k + 2], cy + val[k + 3]);
			...
			case 'S':
				reflectCtrl = (lastCmd == 'c' || lastCmd == 's' || lastCmd == 'C' || lastCmd == 'S');
				path.curveTo(reflectCtrl ? 2 * cx - cpx : cx, reflectCtrl ? 2 * cy - cpy : cy, val[k + 0], val[k + 1],
						val[k + 2], val[k + 3]);
				cpx = (val[k + 0]);
				cpy = (val[k + 1]);
				cx = val[k + 2];
			...
			case 'q':
				...
				cpx = cx + val[k + 0];
				cpy = cy + val[k + 1];
				// Note that we have to update cpx first, since cx will be
				// updated here.
				cx += val[k + 2];
				cy += val[k + 3];
				break;
			...
			case 'a':
				...
				drawArc(path, cx, cy, val[k + 5] + cx, val[k + 6] + cy, val[k + 0], val[k + 1], val[k + 2],
						val[k + 3] != 0, val[k + 4] != 0);
				cx += val[k + 5];
				cy += val[k + 6];
				cpx = cx;
				...
				break;
			case 'A':
				drawArc(path, cx, cy, val[k + 5], val[k + 6], val[k + 0], val[k + 1], val[k + 2], val[k + 3] != 0,
						val[k + 4] != 0);
				cx = val[k + 5];
				cy = val[k + 6];
				cpx = cx;
	...
	private static void drawArc(Path2D p, float x0, float y0, float x1, float y1, float a, float b, float theta,
		...
		logger.log(Level.FINE, "cx, cy, a, b, x0, y0, thetaD, eta0, sweep = " + cx + " , " + cy + " , " + a + " , " + b
				+ " , " + x0 + " , " + y0 + " , " + Math.toDegrees(thetaD) + " , " + Math.toDegrees(eta0) + " , "
	 ...
	 * @param theta
	 *            The angle that the ellipse bounding rectangle makes with the
	 *            horizontal plane
	 * @param start
	 *            The start angle of the arc on the ellipse
	 * @param sweep
	 *            The angle (positive or negative) of the sweep of the arc on
	 *            the ellipse
	 */
	private static void arcToBezier(Path2D p, double cx, double cy, double a, double b, double e1x, double e1y,
