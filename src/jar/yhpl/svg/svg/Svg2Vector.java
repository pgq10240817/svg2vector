...
public class Svg2Vector {
	...
	public static final HashMap<String, String> presentationMap = new HashMap<String, String>() {
		{
	...
	public static HashSet<String> asSet(String... values) {
		return new HashSet<String>(java.util.Arrays.asList(values));
	}

	// List all the Svg nodes that we don't support. Categorized by the types.
	private static final HashSet<String> unsupportedSvgNodes = asSet(
			// Animation elements
			"animate", "animateColor", "animateMotion", "animateTransform",
			"mpath",
			"set",
			// Container elements
			"a", "defs", "glyph", "marker", "mask", "missing-glyph", "pattern",
			"switch",
			"symbol",
			// Filter primitive elements
			"feBlend", "feColorMatrix", "feComponentTransfer", "feComposite", "feConvolveMatrix", "feDiffuseLighting",
			"feDisplacementMap", "feFlood", "feFuncA", "feFuncB", "feFuncG", "feFuncR", "feGaussianBlur", "feImage",
			"feMerge", "feMergeNode", "feMorphology", "feOffset", "feSpecularLighting", "feTile", "feTurbulence",
			// Font elements
			"font", "font-face", "font-face-format", "font-face-name", "font-face-src", "font-face-uri", "hkern",
			"vkern",
			// Gradient elements
	...
	private static SvgTree parse(File f) throws Exception {
		...
		System.out.println("parse cost:"+(System.currentTimeMillis() -time));
		NodeList nSvgNode;
		...
		// TODO: Properly handle transformation in the group level. In the "use"
		// case, we treat
	...
	private static void traverseSVGAndExtract(SvgTree svgTree, SvgGroupNode currentGroup, Node item) {
		...
		for (int i = 0; i < allChildren.getLength(); i++) {
			...
			if (SVG_PATH.equals(nodeName) || SVG_RECT.equals(nodeName) || SVG_CIRCLE.equals(nodeName)
					|| SVG_POLYGON.equals(nodeName) || SVG_LINE.equals(nodeName)) {
				...
				// For other fancy tags, like <refs>, they can contain children
				// too.
	...
	private static void extractRectItem(SvgTree avg, SvgLeafNode child, Node currentGroupNode) {
		...
		if (currentGroupNode.getNodeType() == Node.ELEMENT_NODE) {
			...
			if (!pureTransparent && avg != null && !Float.isNaN(x) && !Float.isNaN(y) && !Float.isNaN(width)
					&& !Float.isNaN(height)) {
	...
	private static void extractLineItem(SvgTree avg, SvgLeafNode child, Node currentGroupNode) {
		...
		if (currentGroupNode.getNodeType() == Node.ELEMENT_NODE) {
			...
			if (!pureTransparent && avg != null && !Float.isNaN(x1) && !Float.isNaN(y1) && !Float.isNaN(x2)
					&& !Float.isNaN(y2)) {
				// "M x1, y1 L x2, y2"
				PathBuilder builder = new PathBuilder();
			...
			for (int k = parts.length - 1; k >= 0; k--) {
				...
				if (nameValue.length == 2 && nameValue[0] != null && nameValue[1] != null) {
					if (presentationMap.containsKey(nameValue[0])) {
						...
						// TODO: This is hacky, since we don't have a group
						// level
						// android:opacity. This only works when the path didn't
						// overlap.
	...
	public static String parseSvgToXml(File inputSVG, OutputStream outStream) {
		// Write all the error message during parsing into SvgTree. and return
		// here as getErrorLog().
