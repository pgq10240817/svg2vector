...
import jar.yhpl.svg.svg.PositionXmlParser.Position;
...
class SvgTree {
	...
	// Although we keep all of the error logs, we only display a limited number
	// of them to keep the
	// UI better looking.
	...
	public void logErrorLine(String s, Node node, SvgLogLevel level) {
		if (s != null && s.length() > 0) {
			if (node != null) {
				...
				mErrorLines.add(level.name() + "@ line " + (position.getLine() + 1) + " " + s + "\n");
			} else {
				mErrorLines.add(s);
			}
		}
