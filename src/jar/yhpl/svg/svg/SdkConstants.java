 ...
 * <ul>
 * <li><code>OS_</code> OS path constant. These paths are different depending on
 * the platform.</li>
...
// Not documenting all the fields here
public final class SdkConstants {
	...
	/**
	 * Returns current platform, one of {@link #PLATFORM_WINDOWS},
	 * {@link #PLATFORM_DARWIN}, {@link #PLATFORM_LINUX} or
	 * {@link #PLATFORM_UNKNOWN}.
	 */
	...
	/**
	 * Property in local.properties file that specifies the path of the Android
	 * SDK.
	 */
	...
	/**
	 * Property in gradle-wrapper.properties file that specifies the URL to the
	 * correct Gradle distribution.
	 */
	 ...
	 * <p>
	 * When possible, use the APIs which take a {@link java.nio.charset.Charset}
	 * and pass in {@link com.google.common.base.Charsets#UTF_8} instead of
	 * using the String encoding method.
	 */
	...
	public static final String FN_DX = "dx" + ext(".bat", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** aapt executable (with extension for the current OS) */
	public static final String FN_AAPT = "aapt" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** aidl executable (with extension for the current OS) */
	public static final String FN_AIDL = "aidl" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** renderscript executable (with extension for the current OS) */
	public static final String FN_RENDERSCRIPT = "llvm-rs-cc" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** renderscript support exe (with extension for the current OS) */
	public static final String FN_BCC_COMPAT = "bcc_compat" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** renderscript support linker for ARM (with extension for the current OS) */
	public static final String FN_LD_ARM = "arm-linux-androideabi-ld" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** renderscript support linker for X86 (with extension for the current OS) */
	public static final String FN_LD_X86 = "i686-linux-android-ld" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** renderscript support linker for MIPS (with extension for the current OS) */
	public static final String FN_LD_MIPS = "mipsel-linux-android-ld" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** adb executable (with extension for the current OS) */
	public static final String FN_ADB = "adb" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** emulator executable for the current OS */
	public static final String FN_EMULATOR = "emulator" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** zipalign executable (with extension for the current OS) */
	public static final String FN_ZIPALIGN = "zipalign" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** dexdump executable (with extension for the current OS) */
	public static final String FN_DEXDUMP = "dexdump" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** proguard executable (with extension for the current OS) */
	public static final String FN_PROGUARD = "proguard" + ext(".bat", ".sh"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** find_lock for Windows (with extension for the current OS) */
	public static final String FN_FIND_LOCK = "find_lock" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	/** hprof-conv executable (with extension for the current OS) */
	public static final String FN_HPROF_CONV = "hprof-conv" + ext(".exe", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	...
	/**
	 * default proguard config file with new file extension (for project
	 * specific stuff)
	 */
	 ...
	 * <p/>
	 * Note: this is not the same as {@link #FD_PKG_SOURCES} which is an SDK
	 * sources folder for packages.
	 */
	...
	/**
	 * Default native library folder name inside the project, i.e. "libs" While
	 * the folder inside the .apk is "lib", we call that one libs because that's
	 * what we use in ant for both .jar and .so and we need to make the 2
	 * development ways compatible.
	 */
	 ...
	 * <p/>
	 * Note this is not the same as {@link #FD_SOURCES} which is the folder name
	 * where sources are installed inside a project.
	 */
	...
	/**
	 * Name of an extra's sample folder. Ideally extras should have one
	 * {@link #FD_SAMPLES} folder containing one or more sub-folders (one per
	 * sample). However some older extras might contain a single "sample" folder
	 * with directly the samples files in it. When possible we should encourage
	 * extras' owners to move to the multi-samples format.
	 */
	...
	/**
	 * Name of the android sources directory and the root of the SDK sources
	 * package folder.
	 */
	 ...
	 */
	public static final String NS_RESOURCES = "http://schemas.android.com/apk/res/android"; //$NON-NLS-1$

	/**
	 * Namespace pattern for the custom resource XML, i.e.
	 * "http://schemas.android.com/apk/res/%s"
	 * <p/>
	 * This string contains a %s. It must be combined with the desired Java
	 * package, e.g.:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 *
	 * Note: if you need an URI specifically for the "android" namespace,
	 * consider using {@link SdkConstants#NS_RESOURCES} instead.
	 */
	...
	/**
	 * Path of the documentation directory relative to the sdk folder. This is
	 * an OS path, ending with a separator.
	 */
	public static final String OS_SDK_DOCS_FOLDER = FD_DOCS + File.separator;

	/**
	 * Path of the tools directory relative to the sdk folder, or to a platform
	 ...
	 * folder. This is an OS path, ending with a separator.
	 */
	public static final String OS_SDK_TOOLS_LIB_FOLDER = OS_SDK_TOOLS_FOLDER + FD_LIB + File.separator;

	/**
	 * Path of the lib directory relative to the sdk folder, or to a platform
	 * folder. This is an OS path, ending with a separator.
	 */
	public static final String OS_SDK_TOOLS_LIB_EMULATOR_FOLDER = OS_SDK_TOOLS_LIB_FOLDER + "emulator" + File.separator; //$NON-NLS-1$

	/**
	 * Path of the platform tools directory relative to the sdk folder. This is
	 * an OS path, ending with a separator.
	 */
	public static final String OS_SDK_PLATFORM_TOOLS_FOLDER = FD_PLATFORM_TOOLS + File.separator;

	/**
	 * Path of the build tools directory relative to the sdk folder. This is an
	 * OS path, ending with a separator.
	 */
	public static final String OS_SDK_BUILD_TOOLS_FOLDER = FD_BUILD_TOOLS + File.separator;

	/**
	 * Path of the Platform tools Lib directory relative to the sdk folder. This
	 * is an OS path, ending with a separator.
	 */
	public static final String OS_SDK_PLATFORM_TOOLS_LIB_FOLDER = OS_SDK_PLATFORM_TOOLS_FOLDER + FD_LIB
			+ File.separator;
	 ...
	 */
	public static final String OS_SDK_TOOLS_PROGUARD_BIN_FOLDER = OS_SDK_TOOLS_FOLDER + "proguard" + File.separator + //$NON-NLS-1$
	 ...
	 * This is an OS path, ending with a separator.
	 */
	public static final String OS_SDK_TOOLS_TEMPLATES_GRADLE_WRAPPER_FOLDER = OS_SDK_TOOLS_FOLDER + FD_TEMPLATES
			+ File.separator + FD_GRADLE_WRAPPER + File.separator;

	/* Folder paths relative to a platform or add-on folder */

	/**
	 * Path of the images directory relative to a platform or addon folder. This
	 * is an OS path, ending with a separator.
	 */
	public static final String OS_IMAGES_FOLDER = FD_IMAGES + File.separator;

	/**
	 * Path of the skin directory relative to a platform or addon folder. This
	 * is an OS path, ending with a separator.
	 */
	...
	/**
	 * Path of the data directory relative to a platform folder. This is an OS
	 * path, ending with a separator.
	 */
	public static final String OS_PLATFORM_DATA_FOLDER = FD_DATA + File.separator;

	/**
	 * Path of the renderscript directory relative to a platform folder. This is
	 * an OS path, ending with a separator.
	 */
	public static final String OS_PLATFORM_RENDERSCRIPT_FOLDER = FD_RENDERSCRIPT + File.separator;

	/**
	 * Path of the samples directory relative to a platform folder. This is an
	 * OS path, ending with a separator.
	 */
	public static final String OS_PLATFORM_SAMPLES_FOLDER = FD_SAMPLES + File.separator;

	/**
	 * Path of the resources directory relative to a platform folder. This is an
	 * OS path, ending with a separator.
	 */
	public static final String OS_PLATFORM_RESOURCES_FOLDER = OS_PLATFORM_DATA_FOLDER + FD_RES + File.separator;

	/**
	 * Path of the fonts directory relative to a platform folder. This is an OS
	 * path, ending with a separator.
	 */
	public static final String OS_PLATFORM_FONTS_FOLDER = OS_PLATFORM_DATA_FOLDER + FD_FONTS + File.separator;

	/**
	 * Path of the android source directory relative to a platform folder. This
	 * is an OS path, ending with a separator.
	 */
	...
	/**
	 * Path of the Ant build rules directory relative to a platform folder. This
	 * is an OS path, ending with a separator.
	 */
	...
	public static final String OS_PLATFORM_ATTRS_XML = OS_PLATFORM_RESOURCES_FOLDER + SdkConstants.FD_RES_VALUES
			+ File.separator + FN_ATTRS_XML;

	/** Path of the attrs_manifest.xml file relative to a platform folder. */
	public static final String OS_PLATFORM_ATTRS_MANIFEST_XML = OS_PLATFORM_RESOURCES_FOLDER
			+ SdkConstants.FD_RES_VALUES + File.separator + FN_ATTRS_MANIFEST_XML;
	...
	/**
	 * Path of the renderscript (clang) include folder relative to a platform
	 * folder.
	 */
	public static final String OS_FRAMEWORK_RS_CLANG = FN_FRAMEWORK_RENDERSCRIPT + File.separator
			+ FN_FRAMEWORK_INCLUDE_CLANG;

	/* Folder paths relative to a addon folder */
	/**
	 * Path of the images directory relative to a folder folder. This is an OS
	 * path, ending with a separator.
	 */
	...
	public static final String PROP_SDK_ANT_TEMPLATES_REVISION = "sdk.ant.templates.revision"; //$NON-NLS-1$
	...
	public static final String CLASS_INSTRUMENTATION_RUNNER = "android.test.InstrumentationTestRunner"; //$NON-NLS-1$
	...
	public static final String CLASS_VIEWGROUP_LAYOUTPARAMS = CLASS_VIEWGROUP + "$" + CLASS_NAME_LAYOUTPARAMS; //$NON-NLS-1$
	public static final String CLASS_NAME_FRAMELAYOUT = "FrameLayout"; //$NON-NLS-1$
	public static final String CLASS_FRAMELAYOUT = "android.widget." + CLASS_NAME_FRAMELAYOUT; //$NON-NLS-1$
	...
	public static final String CLASS_PREFERENCES = "android.preference." + CLASS_NAME_PREFERENCE_SCREEN; //$NON-NLS-1$
	...
	/**
	 * MockView is part of the layoutlib bridge and used to display classes that
	 * have no rendering in the graphical layout editor.
	 */
	...
	/**
	 * Returns the appropriate name for the 'android' command, which is
	 * 'android.exe' for Windows and 'android' for all other platforms.
	 */
	...
	/**
	 * Returns the appropriate name for the 'mksdcard' command, which is
	 * 'mksdcard.exe' for Windows and 'mkdsdcard' for all other platforms.
	 */
	...
	/**
	 * The default prefix used for the {@link #ANDROID_URI} name space including
	 * the colon
	 */
	...
	/** Namespace used in XML files for Android attributes */
	public static final String ANDROID_URI = "http://schemas.android.com/apk/res/android"; //$NON-NLS-1$
	...
	public static final String ATTR_IMPORTANT_FOR_ACCESSIBILITY = "importantForAccessibility"; //$NON-NLS-1$
	public static final String ATTR_LIST_PREFERRED_ITEM_PADDING_LEFT = "listPreferredItemPaddingLeft"; //$NON-NLS-1$
	public static final String ATTR_LIST_PREFERRED_ITEM_PADDING_RIGHT = "listPreferredItemPaddingRight"; //$NON-NLS-1$
	public static final String ATTR_LIST_PREFERRED_ITEM_PADDING_START = "listPreferredItemPaddingStart"; //$NON-NLS-1$
	public static final String ATTR_LIST_PREFERRED_ITEM_PADDING_END = "listPreferredItemPaddingEnd"; //$NON-NLS-1$
	...
	public static final String VALUE_SELECTABLE_ITEM_BACKGROUND = "?android:attr/selectableItemBackground"; //$NON-NLS-1$
	...
	public static final String CLASS_FOLDER = "bin" + File.separator + "classes"; //$NON-NLS-1$ //$NON-NLS-2$
	...
	public static final String ANDROID_CONTENT_CONTENT_PROVIDER = "android/content/ContentProvider"; //$NON-NLS-1$
	public static final String ANDROID_CONTENT_BROADCAST_RECEIVER = "android/content/BroadcastReceiver"; //$NON-NLS-1$
	...
	// Gravity values. These have the GRAVITY_ prefix in front of value because
	// we already
	// have VALUE_CENTER_HORIZONTAL defined for layouts, and its definition
	// conflicts
	...
	public static final String[] TAGS_DATA_BINDING = new String[] { TAG_VARIABLE, TAG_IMPORT, TAG_LAYOUT, TAG_DATA };
	public static final String[] ATTRS_DATA_BINDING = new String[] { ATTR_NAME, ATTR_TYPE, ATTR_CLASS, ATTR_ALIAS };
}